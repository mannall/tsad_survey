...
class KMeans_AD(BaseEstimator, OutlierMixin):
    def __init__(self, k: int = 65, window_size: int = 100, stride: int = 1):
        self.k = k
    ...
    def score(self, test_ts: np.ndarray) -> np.ndarray:
        test_ts = self._preprocess_data(test_ts)
        self.model.fit(test_ts)
        clusters = self.model.predict(test_ts)
        diffs = np.linalg.norm(test_ts - self.model.cluster_centers_[clusters], axis=1)
